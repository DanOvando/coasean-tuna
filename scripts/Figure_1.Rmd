...
### Results directories ----------
results_dir <- paste0(here::here("results", "v5.2"))
...
#https://www.marineregions.org/download_file.php?name=World_EEZ_v11_20191118.zip
...
bet_catch_plot <- ggplot(catch_plot_dat_ordered, aes(x = name, y = catch, fill = gear_long))+
  ...
  scale_fill_manual(name = "Gear Type", values = vesselPalette)+
...
pies <- map(vessel_plot_dat_world_list, function(dat){
  ...
  ggplot(dat, aes(x=1, vessels, fill=gear_type_factor), alpha = 0.7) +
...
pie_tibble_fix <- cbind(pie_tibble, st_coordinates(pie_tibble)) %>%
  mutate(hw = sqrt(tot_vessels)*4.5,
